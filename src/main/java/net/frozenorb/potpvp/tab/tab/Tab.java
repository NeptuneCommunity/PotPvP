...
import net.frozenorb.potpvp.nametag.framework.util.ScoreboardTeamPacketMod;
...
public final class Tab {
    ...
    protected void update() {
        ...
        if (TabHandler.getLayoutProvider() != null) {
            TabLayout tabLayout = TabHandler.getLayoutProvider().provide(this.player);
            if (tabLayout == null) {
                if (this.initiated) {
                    this.reset();
                }
            ...
            for (int y = 0; y < TabLayout.HEIGHT; ++y) {
                for (int x = 0; x < TabLayout.WIDTH; ++x) {
                    ...
                    if (!this.previousPings.containsKey(entryName) || this.pingToBars(this.previousPings.get(entryName)) == this.pingToBars(ping))
                        continue;
