...
public final class MatchHandler {
    ...
    public MatchHandler() {
        ...
        Bukkit.getPluginManager().registerEvents(new MatchBridgesListener(), PotPvPSI.getInstance());
        ...
        for (MatchTeam team : teams) {
            for (UUID member : team.getAllMembers()) {
                ...
                if (isPlayingOrSpectatingMatch(memberPlayer)) {
                    throw new IllegalArgumentException(PotPvPSI.getInstance().uuidCache.name(member) + " is already in a match!");
                }
            }
        }
